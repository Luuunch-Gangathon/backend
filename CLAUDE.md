...
The repo ships with a **single template endpoint** (`GET /raw-materials`) that demonstrates the full layering: schema → fixture → repo → router → main. Every new endpoint should follow the same shape.
...
3. **Repo** — `app/data/repo.py` exposes a plain function (e.g. `list_raw_materials(...)`) that merges fixtures with optional DB rows. DB rows are namespaced with `_db` (e.g. `rm_db_<n>`) so they never collide with fixture IDs.
...
- IDs are strings: `rm_*`, `co_*`. Never raw ints on the wire.
